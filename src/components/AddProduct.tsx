...
export default function AddProduct({ onBack, onSuccess }: AddProductProps) {
  ...
  const [colors, setColors] = useState<string[]>([]); // Starts empty like the screenshot
  const [materials, setMaterials] = useState<string[]>(['Soie 100%']);
  ...
  const [publishStatus, setPublishStatus] = useState<'immediate' | 'draft'>('draft');
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const productDraft: Partial<Product> = {
      ...
      images: finalImages.map((url) => ({ url, width: 600, height: 800 })),
      isAvailable: publishStatus === 'immediate' && Number(stock) > 0,
      tags: [brand, gender, category, publishStatus === 'draft' ? 'brouillon' : ''].filter(Boolean),
  ...
  return (
    <div className="absolute inset-0 bg-[#F4EFE6] text-[#111111] flex flex-col z-30 selection:bg-[#C5A850] selection:text-black font-sans overflow-hidden">
      
      {/* 1. Stationary Sticky Header (Strict Alignment, No Shifting) */}
      <header className="relative w-full h-auto shrink-0 flex items-center justify-center border-b border-[#E3DDD0]/80 bg-[#F4EFE6] select-none z-40 pt-[45px] pb-3.5">
        <button 
          type="button"
          onClick={onBack}
      ...
      <div className="flex-grow overflow-y-auto px-6 py-8 flex flex-col items-center">
                ...
                >
                  ...
                  <label htmlFor="media-file-input" className="cursor-pointer flex flex-col items-center justify-center w-full h-full">
                    ...
                    <span className="text-zinc-400 text-xs font-light max-w-[260px] leading-relaxed block">
                      Glissez-déposez ou cliquez pour parcourir les fichiers (JPG, PNG, MP4).
                    </span>
                  </label>
                </div>
                    ...
                    {isAddingSize ? (
                      <div className="flex items-center gap-1.5 animate-fadeIn">
                        ...
                        <button
                          ...
                          className="bg-[#C5A850] text-white p-1 rounded-lg text-xs"
                        >
                    ...
                    {sizeOptions.map(sz => {
                      ...
                      return (
                        <button
                          ...
                          className={`h-10 min-w-[50px] px-3.5 text-xs flex items-center justify-center rounded-xl border transition-all select-none cursor-pointer font-bold duration-200 ${
                            isSelected
                              ? 'bg-[#C5A850] text-white border-[#C5A850] shadow-md shadow-[#C5A850]/20'
                              : 'bg-[#FCFBF9] border-[#E3DDD0] text-zinc-600 hover:border-[#C5A850] hover:bg-white'
                          }`}
                        >
                          {sz}
                        </button>
      ...
      {/* 4. Stationary Footer Actions (Strictly Pinned, Zero Layout Shifting/Décalage) */}
      <footer className="w-full h-20 shrink-0 border-t border-[#E3DDD0] bg-[#F4EFE6] px-6 flex items-center justify-between select-none z-40">
        <div>
          {step > 1 ? (
            <button
              type="button"
              onClick={() => setStep(prev => prev - 1)}
              className="px-6 py-3 border border-[#E3DDD0] text-zinc-600 hover:text-[#111111] hover:border-black/30 rounded-xl uppercase tracking-widest text-[10px] font-bold font-mono transition-all duration-300 cursor-pointer bg-white/40 hover:bg-white"
            >
              Retour
            </button>
          ) : (
            <div className="w-[85px]" /> /* Balanced spacer */
          )}
        </div>
        ...
        <div className="relative group">
          ...
          <button
            type="button"
            onClick={step === 4 ? () => handleSubmit() : () => setStep(prev => prev + 1)}
          ...
          >
            <span>{step === 4 ? "Publier" : "Continuer"}</span>
          </button>
        </div>
      </footer>
