...
function saveAmountInDinars(value: string) {
  ...
  return amount ? `${amount} DA` : '—';
}

function displayAmountInDinars(value?: string) {
  if (!value || value === '—') return '—';
  const amount = value.replace(/€/g, 'DA').replace(/\b(?:EUR|DZD)\b/gi, 'DA').trim();
  return /\bDA\b/i.test(amount) ? amount : `${amount} DA`;
}
...
export default function Messagerie({
  ...
  const orderBoutiqueId = isClient ? '' : (activeBoutique?.id || '');
  const userIdentifier = isClient
    ? getStableChatUserId(currentUser)
    : activeBoutique?.id || "";

  // Load manual orders and product references from Firestore.
  useEffect(() => {
    const loadOrderData = async () => {
      ...
      try {
        ...
        if (productsResponse.ok) {
          ...
          setProducts(liveProducts.filter((product) => product.boutiqueId === orderBoutiqueId));
        }
        localStorage.removeItem('storehub_manual_orders');
      } catch (error) {
    ...
    const fetchThreads = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setMessagesError("Reconnectez-vous avec le compte Firebase autorisé pour consulter ces messages.");
        }
      } catch (error) {
        console.error("Failed to load chat threads in inbox:", error);
        setMessagesError(error instanceof Error ? error.message : "La messagerie sécurisée est indisponible.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddOrder = async (e: React.FormEvent) => {
    ...
    const normalizedDescription = newDescription.trim().toLocaleLowerCase('fr');
    ...
    const newOrder: ManualOrder = {
      ...
      amount: saveAmountInDinars(newAmount),
      status: newStatus,
      createdAt: new Date().toISOString(),
      boutiqueId: orderBoutiqueId,
  ...
  const handleDeleteOrder = async (orderId: string) => {
    if (window.confirm("Voulez-vous supprimer cette commande manuelle ?")) {
      try {
        const response = await firebaseAuthenticatedFetch(`/api/orders/${orderId}?boutiqueId=${encodeURIComponent(orderBoutiqueId)}`, { method: 'DELETE' });
        if (!response.ok) throw new Error('Order could not be deleted');
        ...
        <>
          ...
          <div className="flex-grow overflow-y-auto px-5 py-4 space-y-3 no-scrollbar">
            {loading && threads.length === 0 ? (
              ...
              <div className="py-16 text-center space-y-3 max-w-[300px] mx-auto" role="alert">
                <AlertCircle className="w-6 h-6 text-amber-400 mx-auto" />
                <p className="text-[11px] text-amber-200 font-light leading-relaxed">{messagesError}</p>
              </div>
        ...
        <div className="flex-grow flex flex-col overflow-hidden">
          ...
          <div className="flex-grow overflow-y-auto px-5 py-4 space-y-3.5 no-scrollbar">
            {filteredOrders.length === 0 ? (
              <div className="w-full max-w-[320px] mx-auto px-5 py-12 text-center space-y-4 bg-transparent border border-dashed border-luxury-gold/35">
                <div className="w-12 h-12 bg-transparent border border-dashed border-luxury-gold/35 flex items-center justify-center text-zinc-600 mx-auto">
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2 border-b border-luxury-border/40 pb-2.5">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          {order.source === 'reservation' && (
                            <span className="border border-luxury-gold/40 bg-luxury-gold/10 px-1.5 py-0.5 font-mono text-[7px] font-bold uppercase tracking-wider text-luxury-gold">
                              Réservation client
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <span className="font-mono text-xs font-semibold text-luxury-gold">
                          {displayAmountInDinars(order.amount)}
                        </span>
                        <button
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                      ...
                      <div className="flex items-center gap-1.5 text-zinc-400 font-mono text-[10px]">
                        ...
                        <span>{order.deliveryDate ? 'Livraison:' : 'Réservé le:'}</span>
          ...
          <div className="bg-luxury-dark border border-luxury-gold/40 w-full max-w-md p-6 space-y-5 shadow-2xl relative">
            ...
            <form onSubmit={handleAddOrder} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  list="storehub-order-products"
                  placeholder="ex: Robe de soirée sur-mesure en soie"
                  value={newDescription}
                  onChange={(e) => setNewDescription(e.target.value)}
                  className="w-full bg-luxury-panel border border-luxury-border text-white text-xs px-3.5 py-2.5 rounded-none focus:outline-none focus:border-luxury-gold transition-colors font-light"
                />
                <datalist id="storehub-order-products">
