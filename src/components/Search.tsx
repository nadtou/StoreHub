...
export default function Search({ onProductClick, favorites, toggleFavorite }: SearchProps) {
  ...
  // Simple customer-facing filters
  const [showFilters, setShowFilters] = useState(false);
  const [selectedAudience, setSelectedAudience] = useState<string>('Tout');
  const [selectedCategory, setSelectedCategory] = useState<string>('Tout');
  const [selectedSize, setSelectedSize] = useState<string>('Tout');
  const [selectedShoeSize, setSelectedShoeSize] = useState<string>('Tout');
  ...
  const audiences = ['Tout', 'Femme', 'Homme', 'Enfant'];
  const categories = [
    { value: 'Tout', label: 'Tous les articles', aliases: [] },
    { value: 'tshirts', label: 'T-shirts et hauts', aliases: ['tshirts', 't-shirts', 'hauts'] },
    { value: 'pantalons', label: 'Pantalons', aliases: ['pantalons', 'pants'] },
    { value: 'robes', label: 'Robes', aliases: ['robes', 'dress'] },
    { value: 'vestes', label: 'Vestes et manteaux', aliases: ['vestes', 'outerwear', 'manteaux'] },
    { value: 'chaussures', label: 'Chaussures', aliases: ['chaussures', 'shoes'] },
    { value: 'accessoires', label: 'Accessoires', aliases: ['accessoires', 'bijoux'] },
  ];
  const clothingSizes = ['Tout', 'XS', 'S', 'M', 'L', 'XL', 'XXL'];
  const shoeSizes = ['Tout', ...Array.from({ length: 34 }, (_, index) => String(index + 16))];
  const colors = ['Tout', 'Noir', 'Blanc', 'Beige', 'Marron', 'Gris', 'Bleu', 'Rouge', 'Vert', 'Rose', 'Jaune', 'Orange', 'Violet'];
  const resetFilters = () => {
    setSelectedAudience('Tout');
    setSelectedCategory('Tout');
    setSelectedSize('Tout');
    setSelectedShoeSize('Tout');
    setSelectedColor('Tout');
    setMaxPrice(0);
  };

  const activeFiltersCount = [
    selectedAudience !== 'Tout',
    selectedCategory !== 'Tout',
    selectedSize !== 'Tout',
    selectedShoeSize !== 'Tout',
    selectedColor !== 'Tout',
    maxPrice > 0,
  ...
  const filteredProducts = products.filter(p => {
    const productSizes = (p.sizes || []).map(String);
    ...
    const matchesKeyword = searchQuery === '' || 
      ...
      productKeywords.some(kw => kw.toLowerCase().includes(searchQuery.toLowerCase()));

    const category = categories.find((item) => item.value === selectedCategory);
    const matchesAudience = selectedAudience === 'Tout'
      || productTags.some((tag) => tag.toLowerCase() === selectedAudience.toLowerCase());
    const matchesCategory = selectedCategory === 'Tout'
      || Boolean(category?.aliases.includes((p.category || '').toLowerCase()));
    const matchesSize = selectedSize === 'Tout' || productSizes.includes(selectedSize);
    const matchesShoeSize = selectedShoeSize === 'Tout' || productSizes.includes(selectedShoeSize);
    const matchesColor = selectedColor === 'Tout' || productColors.some(c => c.toLowerCase().includes(selectedColor.toLowerCase()));
    const matchesPrice = maxPrice === 0 || p.price <= maxPrice;

    return matchesKeyword && matchesAudience && matchesCategory && matchesSize && matchesShoeSize && matchesColor && matchesPrice;
  });
  ...
  // Fetch preset images and submit them to the real visual-search endpoint.
  const handlePresetSelect = async (presetUrl: string) => {
    ...
    try {
      ...
      console.error('Preset visual search failed:', err);
      setApiError("L’image d’exemple n’a pas pu être analysée. Réessayez ou choisissez une photo depuis votre téléphone.");
      setIsAnalyzing(false);
    }
  };
        ...
        <div className="max-w-6xl mx-auto px-6 mt-8">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              aria-controls="customer-search-filters"
            ...
            >
              ...
              {activeFiltersCount > 0 && (
                <span className="flex h-5 min-w-5 items-center justify-center bg-luxury-gold px-1 text-[10px] font-bold text-black">
                  {activeFiltersCount}
                </span>
              )}
            </button>
          ...
          {/* Simple customer filter drawer */}
          {showFilters && (
            <section
              id="customer-search-filters"
              aria-label="Filtres de recherche"
              className="mt-4 border border-luxury-border bg-[#0d0d0d] p-4 sm:p-5"
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-sm font-semibold text-white">Filtrer les articles</h3>
                  <p className="mt-1 text-xs text-zinc-500">Des choix simples pour trouver rapidement.</p>
                </div>
                {activeFiltersCount > 0 && (
                  <button
                    type="button"
                    onClick={resetFilters}
                    className="flex shrink-0 items-center gap-1.5 border border-luxury-border px-2.5 py-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-300 transition-colors hover:border-luxury-gold hover:text-luxury-gold"
                  >
              ...
              <fieldset className="mt-5">
                <legend className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Pour qui ?</legend>
                <div className="grid grid-cols-4 gap-1.5">
                  {audiences.map(audience => (
                    <button
                      ...
                      className={`min-h-10 border px-1 text-[11px] font-medium transition-colors ${
                        selectedAudience === audience
                          ? 'border-luxury-gold bg-luxury-gold text-black'
                          : 'border-luxury-border bg-luxury-panel text-zinc-300 hover:border-zinc-500'
                      }`}
                    >
                      {audience}
                    </button>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <label className="col-span-2 block">
                  <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Type d’article</span>
                  <select
                    value={selectedCategory}
                    onChange={e => {
                      const nextCategory = e.target.value;
                      setSelectedCategory(nextCategory);
                      if (nextCategory === 'chaussures') {
                        setSelectedSize('Tout');
                      } else {
                        setSelectedShoeSize('Tout');
                      }
                    }}
                    className="min-h-11 w-full border border-luxury-border bg-luxury-panel px-3 text-sm text-white outline-none transition-colors focus:border-luxury-gold"
                  >
                ...
                {selectedCategory === 'chaussures' ? (
                  <label className="block">
                    <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Pointure</span>
                    <select
                      value={selectedShoeSize}
                      onChange={e => setSelectedShoeSize(e.target.value)}
                      className="min-h-11 w-full border border-luxury-border bg-luxury-panel px-3 text-sm text-white outline-none transition-colors focus:border-luxury-gold"
                    >
                      {shoeSizes.map(size => <option key={size} value={size}>{size === 'Tout' ? 'Toutes' : size}</option>)}
                    </select>
                  </label>
                ) : (
                  <label className="block">
                    <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Taille</span>
                    <select
                      value={selectedSize}
                      onChange={e => setSelectedSize(e.target.value)}
                      className="min-h-11 w-full border border-luxury-border bg-luxury-panel px-3 text-sm text-white outline-none transition-colors focus:border-luxury-gold"
                    >
                      {clothingSizes.map(size => <option key={size} value={size}>{size === 'Tout' ? 'Toutes' : size}</option>)}
                    </select>
                  </label>
                )}

                <label className="block">
                  <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Teinte</span>
                  <select
                    value={selectedColor}
                    onChange={e => setSelectedColor(e.target.value)}
                    className="min-h-11 w-full border border-luxury-border bg-luxury-panel px-3 text-sm text-white outline-none transition-colors focus:border-luxury-gold"
                  >
                    {colors.map(color => <option key={color} value={color}>{color === 'Tout' ? 'Toutes' : color}</option>)}
                  </select>
                </label>

                <label className="col-span-2 block">
                  <span className="mb-2 block text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Budget maximum</span>
                  <div className="relative">
                    <input
                      ...
                      onChange={e => {
                        ...
                        setMaxPrice(Number.isFinite(amount) ? Math.max(0, amount) : 0);
                      }}
                      placeholder="Exemple : 150"
                      className="min-h-11 w-full border border-luxury-border bg-luxury-panel px-3 pr-11 text-sm text-white outline-none transition-colors placeholder:text-zinc-600 focus:border-luxury-gold"
                    />
                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-luxury-gold">DA</span>
                  </div>
                </label>
              </div>
              ...
              <button
                ...
                className="mt-5 min-h-12 w-full bg-luxury-gold px-4 text-xs font-bold uppercase tracking-widest text-black transition-colors hover:bg-[#f1d35d]"
              >
                Afficher {filteredProducts.length} article{filteredProducts.length > 1 ? 's' : ''}
              </button>
            </section>
          )}
          ...
          <div className="mt-10">
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-24 border border-dashed border-luxury-border rounded-none">
                ...
                <p className="text-xs text-zinc-600">Essayez une autre catégorie ou effacez les filtres.</p>
                {activeFiltersCount > 0 && (
                  <button
                    type="button"
                    onClick={resetFilters}
                    className="mt-5 border border-luxury-gold px-4 py-2.5 text-[10px] font-semibold uppercase tracking-widest text-luxury-gold transition-colors hover:bg-luxury-gold hover:text-black"
                  >
                    Effacer les filtres
                  </button>
                )}
                  ...
                  {visualResult && (
                    <div className="space-y-6">
                      ...
                      <div className="p-4 bg-luxury-gold/5 rounded-none border border-luxury-gold/20">
                        <span className="font-mono text-[9px] uppercase tracking-widest text-luxury-gold block mb-1 font-semibold">Conseil FENNCO IA</span>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-grow">
                              ...
                              <span className="font-mono text-xs text-luxury-gold font-semibold">{p.price} DA</span>
                            </div>
