...
export default function StylistChat({ preferences, onProductClick }: StylistChatProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      sender: 'model',
      text: "Bonjour chéri(e). Je suis Aurelia, votre styliste personnelle chez StoreHub.\n\nJe suis là pour vous aider à sculpter votre vestiaire idéal. Dites-moi : cherchez-vous un ensemble fluide pour une escapade estivale, ou une pièce structurée d'inspiration scandinave ?",
      timestamp: new Date()
    }
  ]);
  ...
  const suggestedPrompts = [
    "Quelle robe de soie irait bien avec des bijoux dorés ?",
    "Propose-moi un look minimaliste et chaud.",
    "Que suggères-tu en lin pour la Riviera ?"
  ];
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Impossible de joindre Aurelia.");
      }
    ...
    setMessages([
      {
        id: 'welcome',
        sender: 'model',
        text: "Bonjour chéri(e). Je suis Aurelia, votre styliste personnelle chez StoreHub.\n\nReprenons notre conversation style sur de nouvelles bases. Qu'aimeriez-vous essayer aujourd'hui ?",
        timestamp: new Date()
      }
    ]);
  };

  // Extract products mentioned in Aurelia's message for quick interaction
    ...
    <div className="max-w-4xl mx-auto px-6 mt-[45px] pb-24 h-[calc(100vh-180px)] flex flex-col justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-mono text-[9px] uppercase tracking-widest text-luxury-gold">
              Styliste Privée StoreHub • En ligne
            </p>
          </div>
        </div>
                  ...
                  {mentioned.map(p => (
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        ...
                        <span className="font-mono text-[10px] text-luxury-gold font-semibold">{p.price} €</span>
                      </div>
        ...
        {isTyping && (
          <div className="flex flex-col items-start animate-pulse">
            <div className="bg-luxury-panel/30 border border-luxury-border rounded-none px-5 py-3.5 text-xs text-zinc-500 font-mono tracking-widest flex items-center gap-2">
              <span>Aurelia ajuste ses croquis</span>
      ...
      <div className="flex gap-3 pt-4 border-t border-luxury-border bg-luxury-dark">
        <input
          ...
          placeholder="Décrivez votre tenue idéale, posez des questions style à Aurelia..."
